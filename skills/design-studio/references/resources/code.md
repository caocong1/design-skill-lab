...
| --- | --- | --- | --- | --- |
| [Anthropic frontend-design skill](https://github.com/anthropics/skills/tree/main/skills/frontend-design) | Anthropic's own short skill for distinctive front-end aesthetics: commit to a bold direction, avoid generic AI looks, mind typography, colour, motion, composition | Read it as the minimal baseline any design skill must beat; the companion post (2025-11-12) explains the mechanism, distributional convergence, and ships a ~400-token aesthetics prompt | free · static · See repository licence | S |
...
| [Impeccable](https://impeccable.style) | One design skill with 24 commands (shape, critique, audit, polish, distill, bolder, quieter, harden, animate...), a PRODUCT.md that records durable product truth, live in-browser variant iteration, and a catalogue of AI-design tells with deterministic detectors that need no LLM | Read /slop for the rule catalogue (each rule is tagged CLI, browser, LLM-only or optional); npx impeccable detect src/ runs the deterministic rules; Chinese edition at impeccable.cn | free · static · Apache-2.0 (github.com/pbakaus/impeccable) | S |
...
| [v0 by Vercel](https://v0.app) | Prompt-to-app generator that builds and deploys full-stack web apps, with a visual design mode, reusable design systems and a large community template gallery (apps, landing pages, components, dashboards) | Use for fast throwaway variants, then apply this suite's brief, direction and critique discipline to the output; browse /templates to see what current AI-generated UI looks like, which is also the look to diverge from | freemium · static · login | A |
| [Impeccable 中文站 (zh)](https://impeccable.cn) | Chinese-language edition of the Impeccable site: docs, the command set, live mode, and the full catalogue of AI-design anti-patterns with how each one is detected | Read /slop (the rule catalogue with a 2022-vs-2026 era toggle), /docs and /designing; it links to the official repository and npm package, and may lag the English site (it lists 23 commands where the repository lists 24) | free · static · Apache-2.0 (github.com/pbakaus/impeccable) | A |
