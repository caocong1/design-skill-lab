...
- **2026-09-24 更新**：第二个项目（ai-assistant-web）也撞上了，做法不同：Playwright 保存登录态，"之前"拍线上、"之后"用 `context.route` 把同源前端换成本地构建。0.7.0 已把这个做法写进 `render-and-look.md` 的工具梯级；脚本仍待定，接口要同时支持"登录 + 路由表"和"线上后端 + 本地前端"两种。
