...
由线上页面的两个 bug 报告触发，顺带把页面从十个方向扩到十五个。过程与教训补记在 `analysis/12-dogfooding-the-lab-page.md`（1.1）。
...
- 五个新方向全部经渲染查看（1280 与 390 宽，域视图与搜索态），所有文字/背景对经 `color_tools.py` 计算 ≥ 4.5:1（初稿有五对不达标，已改色）。十五个方向在同一脚本下逐个挂载、进域、搜索、按 `]` 轮换，无脚本错误。
...
- 新方向仍由作者自评；字样风格在 Windows / Linux 下会有几行落到同一款字体（说明卡已坦白）；线路图的站名只在悬停时显示，靠下方线路表补足。
